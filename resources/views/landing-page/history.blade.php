            ...
            <script>
                ...
                function filterHistories(invoice, status) {
                    return histories.filter(item => {
                        let matchInvoice = !invoice || (item.invoice_number && item.invoice_number.toLowerCase().includes(
                            invoice.toLowerCase()));
                ...
                function renderHistory(datas, currentPage, lastPage) {
                    ...
                    if (datas.length === 0) {
                        ...
                        datas.forEach(function(item, index) {
                            ...
                            if (item.status === 'pending') {
                                statusHtml =
                                    `<span class="inline-flex items-center px-2.5 py-0.5 rounded bg-yellow-100 text-yellow-800 text-xs font-medium">Pending</span>`;
                            } else if (item.status === 'denied') {
                                statusHtml =
                                    `<span class="inline-flex items-center px-2.5 py-0.5 rounded bg-red-100 text-red-800 text-xs font-medium">Denied</span>`;
                            } else if (item.status === 'waiting') {
                                statusHtml =
                                    `<span class="inline-flex items-center px-2.5 py-0.5 rounded bg-blue-100 text-blue-800 text-xs font-medium">Waiting</span>`;
                            } else if (item.status === 'paid') {
                                statusHtml =
                                    `<span class="inline-flex items-center px-2.5 py-0.5 rounded bg-green-100 text-green-800 text-xs font-medium">Paid</span>`;
                            } else {
                                statusHtml =
                                    `<span class="inline-flex items-center px-2.5 py-0.5 rounded bg-gray-100 text-gray-800 text-xs font-medium">${item.status_pembayaran ? item.status_pembayaran.charAt(0).toUpperCase() + item.status_pembayaran.slice(1) : 'Unknown'}</span>`;
                            }
                    ...
                    `;
                                ...
                                actionsHtml =
                                    `<a href="/checkout/${item.id}" class="inline-block px-4 py-2 border border-blue-400 text-blue-700 text-sm rounded-lg hover:bg-blue-50">Lanjut Checkout</a>`;
                            } else if (item.step == 2) {
                                actionsHtml =
                                    `<a href="/checkout/second/${item.id}" class="inline-block px-4 py-2 border border-blue-400 text-blue-700 text-sm rounded-lg hover:bg-blue-50">Lanjut Checkout</a>`;
                            } else if (item.step == 3) {
                                actionsHtml =
                                    `<a href="/checkout/third/${item.id}" class="inline-block px-4 py-2 border border-blue-400 text-blue-700 text-sm rounded-lg hover:bg-blue-50">Lanjut Checkout</a>`;
                            } else if (item.step == 4) {
                                actionsHtml =
                                    `<a href="/checkout/fourth/${item.id}" class="inline-block px-4 py-2 border border-blue-400 text-blue-700 text-sm rounded-lg hover:bg-blue-50">Lanjut Checkout</a>`;
                            } else if (item.step == 5) {
                                actionsHtml = `
                    <form action="/checkout/store" method="POST" id="checkout-form-${item.id}">
                        ...
                        ${(item.orders || []).map(kirim => kirim.pengiriman ? `<input type="hidden" name="id_pengiriman[]" value="${kirim.pengiriman}">` : '').join('')}
                        <input type="hidden" name="total_harga" value="${parseInt(item.total_harga)}">
                        <button type="button" class="inline-block px-4 py-2 border border-blue-400 text-blue-700 text-sm rounded-lg hover:bg-blue-50 cursor-pointer" onclick="confirmPayment(${item.id})">Lanjut Ke Pembayaran</button>
                    </form>
                    `;
                            }
                ...
                function renderPagination(currentPage, lastPage) {
                    ...
                    for (let i = 1; i <= lastPage; i++) {
                        html +=
                            `<button class="mx-1 px-3 py-1 rounded ${i === currentPage ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}" onclick="loadHistory(${i})">${i}</button>`;
                    }
