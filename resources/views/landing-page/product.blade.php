...
@section('content')
    ...
    <section
        ...
        <div class="product_heading flex flex-col w-full md:w-1/2">
            ...
            <p class="mt-3 text-base md:text-lg">Kami hadir untuk memudahkan anda memenuhi kebutuhan spiritual dengan banyak
                pilihan dan pengiriman terpercaya</p>
        </div>
    ...
    <section class="product_filter w-full mb-8 md:mb-10 flex flex-col md:flex-row gap-4 md:gap-0" data-aos="fade-up">
        ...
        <div class="product_category flex flex-wrap py-4 md:py-6 px-4 md:px-10 gap-4 md:gap-6 items-center">
            ...
            <a href="{{ route('product.search', array_merge(request()->except('sort'), ['sort' => 'atoz'])) }}"
                class="cursor-pointer hover:underline {{ request('sort') == 'atoz' ? 'font-bold underline text-[#534538]' : '' }}">
                (A-Z)
            </a>
            <a href="{{ route('product.search', array_merge(request()->except('sort'), ['sort' => 'price_lowest'])) }}"
                class="cursor-pointer hover:underline {{ request('sort') == 'price_lowest' ? 'font-bold underline text-[#534538]' : '' }}">
                (Harga Terendah)
            </a>
            <a href="{{ route('product.search', array_merge(request()->except('sort'), ['sort' => 'price_highest'])) }}"
            ...
            @if ($products->hasPages())
                <nav class="flex items-center gap-2">
                    ...
                    @if ($products->onFirstPage())
                        <span class="w-8 h-8 flex items-center justify-center border border-gray-300 bg-[#faf6ee] text-gray-400 rounded cursor-not-allowed">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                            </svg>
                        </span>
                    @else
                        <a href="{{ $products->previousPageUrl() }}" rel="prev" class="w-8 h-8 flex items-center justify-center border border-gray-300 bg-[#faf6ee] text-black rounded hover:bg-gray-200 transition">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                            </svg>
                        </a>
                    ...
                    @foreach ($products->getUrlRange(1, $products->lastPage()) as $page => $url)
                        @if ($page == $products->currentPage())
                            <span class="w-8 h-8 flex items-center justify-center bg-orange-500 text-black font-bold rounded transition text-base">{{ $page }}</span>
                        @else
                            <a href="{{ $url }}" class="w-8 h-8 flex items-center justify-center border border-gray-300 bg-[#faf6ee] text-gray-400 rounded hover:bg-gray-200 transition text-base">{{ $page }}</a>
                        @endif
                    @endforeach

                    {{-- Next Page Link --}}
                    @if ($products->hasMorePages())
                        <a href="{{ $products->nextPageUrl() }}" rel="next" class="w-8 h-8 flex items-center justify-center border border-gray-300 bg-[#faf6ee] text-black rounded hover:bg-gray-200 transition">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
