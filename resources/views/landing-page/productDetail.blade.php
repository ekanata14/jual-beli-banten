...
<section
    ...
    <div class="product_image w-full lg:w-1/3" data-aos="fade-right" data-aos-delay="200">
        ...
        <!-- <div class="product_image_list w-full flex justify-between mt-5 gap-2 md:gap-4">
            @for ($i = 0; $i < 4; $i++) <img src="{{ asset($product['foto']) }}" alt="{{ $product['nama_produk'] }}"
    ...
    <div class="product_info flex flex-col gap-8 lg:gap-16 w-full lg:w-2/3" data-aos="fade-left" data-aos-delay="300">
        <div class="product_header">
            <h2 class="text-[#1C1917] font-serif text-2xl md:text-4xl font-semibold"
                style="font-weight: 600 !important;">{{ $product['nama_produk'] }}
            </h2>
            @php
            ...
            <div class="mt-4 flex items-center gap-2">
                <div class="flex">
                    @for ($i = 1; $i <= 5; $i++) @if ($averageRating>= $i)
                        <img src="{{ asset('assets/icons/star-full.svg') }}" alt="Star" class="w-4 h-4 text-[#FF7006]">
                        @elseif ($averageRating >= $i - 0.5)
                        <img src="{{ asset('assets/icons/star-half.svg') }}" alt="Star" class="w-4 h-4 text-[#FF7006]">
                        @else
            ...
            <div class="flex items-end mt-3">
                <h2 class="text-[#1C1917] text-xl md:text-3xl font-semibold">
        ...
        <div class="product_details_desc flex flex-col gap-4">
            <div class="product_desc flex flex-col gap-1">
                <p class="text-[#1C1917] font-semibold">Penjual</p>
                <p class="text-[#4B4B4B] text-sm md:text-base leading-relaxed">
                    {{ $product->user->name }}
                </p>
            </div>
        ...
        <div class="product__footer">
            ...
            <div class="flex items-center gap-3 mt-4 mb-8">
                ...
                <input type="number" id="quantity" name="quantity" value="1" min="1" max="{{ $product['stok'] }}"
                    class="w-12 md:w-16 text-center border border-gray-300 rounded-md" readonly>
                ...
                <div class="flex flex-col gap-3 w-full">
                    <form action="{{ route('cart.checkout.direct') }}" method="POST" class="w-full" id="checkoutForm">
                        ...
                        <x-button type="submit" icon="{{ asset('assets/icons/arrow_right_white.svg') }}"
                            class="w-full text-white hover:bg-[#463c31] transition duration-300" id="checkoutBtn">
                            Beli Sekarang
                        </x-button>
                    </form>
                    ...
                    <form action="{{ route('cart.add') }}" method="POST" class="w-full" id="cartForm">
                        ...
                        <button type="submit"
                            class="w-full flex items-center justify-center gap-1 bg-white text-[#1C1917] hover:text-dark transition-colors duration-200 rounded-md py-2 text-sm md:text-base cursor-pointer"
                            id="cartBtn">
...
<section class="home_testi py-26 md:py-40 px-10 w-full" data-aos="fade-up" data-aos-delay="100">
    <div class="home_testi_content flex flex-col items-left gap-2 md:gap-10 mx-auto w-full">
        <div class="home_testi_content_header flex flex-col items-left justify-center w-full md:w-2/3"
            ...
            <h2 class="text-black mt-2 text-lg md:text-2xl font-semibold">
                Pendapat pelanggan tentang produk kami
            </h2>
        </div>
                        ...
                        <div class="testimonial-card_content">
                            <div class="stars flex mb-2">
                                ...
                                @for ($i = 0; $i < $fullStars; $i++) <img
                                    src="{{ asset('assets/icons/star-full.svg') }}" alt="Star" class="w-5 h-5">
                                    @endfor
